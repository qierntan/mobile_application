...
try {
  ...
  console.log('Configuration loaded:', config.payment_server_url);
} catch (error) {
  console.error('Error loading config, using fallback:', error);
  config = { payment_server_url: 'http://192.168.1.103:4242' };
}
...
app.post('/create-checkout-session', async (req, res) => {
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: success_url || `${config.payment_server_url}/success`,
      cancel_url: cancel_url || `${config.payment_server_url}/cancel`,
    });
